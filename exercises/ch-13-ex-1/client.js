...
app.get('/userinfo', function(req, res) {
  ...
  var resouces = request('GET', authServer.userInfoEndpoint, { headers: headers })
  if(resouces.statusCode >= 200 && resouces.statusCode < 300) {
    var body = JSON.parse(resouces.getBody())
    userInfo = body;
    res.render('userinfo', { userInfo: userInfo, id_token: id_token })
    return
  } else {
    res.render('error', { error: 'unable to fetchc user info' })
    return
  }
});
